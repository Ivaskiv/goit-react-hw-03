...
const userSchema = Yup.object().shape({
  ...
  number: Yup.string()
    ...
    .matches( /^(\d{3}-\d{2}-\d{2}|\(\d{3}\) \d{3}-\d{4}|\+\d{1,2}\s\d{3}-\d{2}-\d{2}-\d{2}|\d{10})$/,'Invalid phone number format.')
    .required('This is a required field'),
});
